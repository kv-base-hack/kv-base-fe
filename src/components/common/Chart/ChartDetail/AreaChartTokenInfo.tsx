...
type ChartProps = DataChart[]

export function AreaChartTokenInfo({
  sparkLineIn7D,
  dataTokenInfo,
  loading,
  setValueIndex,
  ...
  const formatData = useMemo(() => {
    return sparkLineIn7D?.map((item: DataChart, index) => {
      return {
        key: index,
        date: moment(item?.time_ms * 1000).format('MMM:DD, HH:mm'),
        price: item.rate,
      }
    })
  }, [sparkLineIn7D])
