...
import { DEFAULT_EXPLORER, FormProps, IInit, IOnRequestIxCallback } from '@/types/jup';
...
export const IntegratedTerminal = (props: {
  rpcUrl: string;
  formProps: FormProps;
  ...
  return (
    <div className="flex justify-center rounded-lg w-full bg-neutral-08 border border-solid border-white/10">
      <div className="flex flex-col lg:flex-row h-full w-full overflow-auto">
        <div className="w-full h-full rounded-lg overflow-hidden flex justify-center">
          {/* Loading state */}
          {!isLoaded ? (
            <div className="h-[400px] w-full animate-pulse bg-white/10 mt-4 lg:mt-0 flex items-center justify-center rounded-lg">
          ...
          <div
            ...
            className={`flex h-full w-full overflow-auto justify-center bg-[#282830] rounded-lg ${
              !isLoaded ? 'hidden' : ''
            }`}
          />
        </div>
