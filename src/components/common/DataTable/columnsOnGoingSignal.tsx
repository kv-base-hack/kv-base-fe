...
export const columnsOngoingSignal: ColumnDef<OnGoingSignal>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="text-neutral-04">{moment(createdAt).format('MMM DD, HH:mm')}</div>
    },
  },
  {
    accessorKey: 'pair',
        ...
        <div className="flex items-center justify-center gap-2 w-24">
          <div className="w-6 h-6">
            {targets?.[0]?.reached ? (
              ...
              <div className="w-6 h-6 border-2 border-solid border-white/10 rounded-full"></div>
            )}
          </div>
          {targets?.[0]?.price ? (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return stopLoss?.price ? <div>{numeral(stopLoss?.price).format('0,0.[0000]')}</div> : '-'
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className={cn(floatingROI > 0 ? 'text-success-500' : 'text-error-500')}>
