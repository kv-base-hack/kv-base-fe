...
import { getFindGemsTopScore } from '@/services/api'

const GET_FIND_GEMS_TOP_SCORE = 'GET_FIND_GEMS_TOP_SCORE'

interface FindGemsTopScore {
  chain?: string
  limit?: number
  start?: number
...
export const useFindGemsTopScore = ({
  chain = CHAIN,
  limit = 10,
  start = 1,
  ...
  queryFn: async () => {
    const data = await getFindGemsTopScore({
      chain,
      limit,
      start,
      price_change_24h_min,
